      ...
      modules: [
        {
          ...
          operations: [
            ...
            {
              id: "op-set-backup-schedule",
              name: "SET_BACKUP_SCHEDULE",
              description:
                "Set or update the scheduled backup configuration for an environment. Stored on state; the observability subgraph polls envs each minute and fires scheduled pg_dump exports per the configured cadence, keeping at most `retention` SCHEDULED dumps. Setting `enabled: false` pauses the schedule without losing settings.",
              schema:
                "input SetBackupScheduleInput {\n  enabled: Boolean!\n  cadence: BackupCadence!\n  retention: Int!\n}",
              template: "",
              reducer:
                'if (action.input.retention < 1 || action.input.retention > 30) {\n  throw new Error("INVALID_RETENTION: retention must be between 1 and 30");\n}\nstate.backupSchedule = {\n  enabled: action.input.enabled,\n  cadence: action.input.cadence,\n  retention: action.input.retention,\n};',
              errors: [
                {
