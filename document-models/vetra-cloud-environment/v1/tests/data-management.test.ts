...
describe("DataManagementOperations", () => {
  ...
  describe("SET_RUNTIME_CONFIG", () => {
    ...
    it("rejects a non-string packageRegistryUrl", () => {
      ...
      document = reducer(document, {
        ...setRuntimeConfig({
          config: JSON.stringify({ packageRegistryUrl: 123 }),
        }),
        ...userSigner(ALICE),
      });
    ...
    it("rejects unknown top-level keys (additionalProperties: false)", () => {
      ...
      document = reducer(document, {
        ...setRuntimeConfig({
          config: JSON.stringify({ app: { logLevel: "debug" } }),
        }),
        ...userSigner(ALICE),
      });
  ...
  describe("SET_STUDIO_INSTANCE", () => {
    // Links this env to the studio that produced it. Pure metadata — owner-gated,
    // no status transition (renders nothing into the chart/gitops values).
    it("sets studioInstanceId to the given studio env id (owner-signed)", () => {
      let document = utils.createDocument();
      document = reducer(document, {
        ...setOwner({ address: ALICE }),
        ...userSigner(ALICE),
      ...
      document = reducer(document, {
        ...setStudioInstance({ studioInstanceId: "studio-env-123" }),
        ...userSigner(ALICE),
      });

      expect(document.state.global.studioInstanceId).toBe("studio-env-123");
      expect(document.operations.global.at(-1)?.error).toBeUndefined();
    });

    it("clears studioInstanceId when given null", () => {
      let document = utils.createDocument();
      document = reducer(document, {
        ...setOwner({ address: ALICE }),
        ...userSigner(ALICE),
      ...
      document = reducer(document, {
        ...setStudioInstance({ studioInstanceId: null }),
        ...userSigner(ALICE),
      });

      expect(document.state.global.studioInstanceId).toBeNull();
    });

    it("rejects a non-owner signer without mutating state", () => {
      let document = utils.createDocument();
      document = reducer(document, {
        ...setOwner({ address: ALICE }),
        ...userSigner(ALICE),
      ...
      expect(document.state.global.studioInstanceId).toBeNull();
      expect(document.operations.global.at(-1)?.error).toMatch(/owner/i);
    });
  });
});
