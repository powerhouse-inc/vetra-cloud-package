...
describe("DataManagementOperations", () => {
  ...
  it("should handle setStudioInstance operation", () => {
    const document = utils.createDocument();
    const input = generateMock(SetStudioInstanceInputSchema());

    const updatedDocument = reducer(document, setStudioInstance(input));
  ...
  it("sets studioInstanceId to the given studio env id", () => {
    const document = utils.createDocument();
    ...
    expect(updatedDocument.state.global.studioInstanceId).toBe(
      "studio-env-123",
    );
  });
  ...
  it("clears studioInstanceId when given null", () => {
    ...
    expect(cleared.state.global.studioInstanceId).toBeNull();
  });
});
